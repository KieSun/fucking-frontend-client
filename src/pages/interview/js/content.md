...
- `undefined、null、false、NaN、''、0、-0` 都专为 `false`。
...
先来说下数据存放的正确规则是：局部、占用空间确定的数据，一般会存放在栈中，否则就在堆中。 那么接下来我们可以通过 Chrome 来帮助我们验证这个说法说法。

![](https://yck-1254263422.cos.ap-shanghai.myqcloud.com/2021/03/27/16168151767374.jpg)

上图中画红框的位置我们能看到一个内部的对象 `[[Scopes]]`，其中存放着变量 `a`，笔者推测该对象是被存放在堆上的，其中包含了闭包、全局对象等等内容，因此我们能通过闭包访问到本该销毁的变量。
...
从上图我们能发现全局下声明的变量，如果是 var 的话就直接被挂到 `globe` 上，如果是其他关键字声明的话就被挂到 `Script` 上，根据 `[[Scopes]]` 存放在堆上的推测，我们又能得出全局变量也是被存储在堆上的。

以上从浏览器中发现的都能验证数据存放规则。
...
因此笔者对于原始类型存储位置的理解为：**只有局部变量才是被存储在栈上，其它都存储在堆上。**
